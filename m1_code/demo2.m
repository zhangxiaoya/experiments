...
A_neg = A_nego;    
num_pf = 10;
preframes = A_pos(:,1:num_pf);   % add zhyn
...
for f = 1:num
    ...
    ppreframes = normVector(preframes);  % add zhyn
    paramSR.L = length(YY(:,1));                       
    paramSR.lambda = 0.01;
    beta1 = mexLasso(YY,ppreframes,paramSR);
    rec_p = sum((YY - ppreframes*beta1).^2);
%     rec_pp = sort(rec_p);
%     idx_rec = find(rec_p >= rec_pp(10));
%     YY = YY(:,idx_rec);
    conn = exp(-rec_p/gamma);
...
%     likelihood = con;
    ...
    [preframes] = affineTrainPF(dataPath,f, sz, opt, param, num_pf, forMat); 
end
